      ...
      {step === 'auth' && (
        <div className="space-y-4">
          <div className="p-4 rounded-lg border border-amber-500/20 bg-amber-500/5 space-y-3">
            ...
            <div className="p-3 rounded bg-black/20 border border-border/20 space-y-2">
              <p className="text-xs text-muted-foreground mb-1.5">Option 1: Run auth from this dashboard</p>
              <Button
                size="sm"
                variant="outline"
                disabled={loginBusy}
                onClick={async () => {
                  ...
                  try {
                    const res = await fetch('/api/agent-runtimes', {
                      method: 'POST',
                      headers: { 'Content-Type': 'application/json' },
                      body: JSON.stringify({ action: 'login', runtime: 'codex' }),
                    })
                    const data = await res.json()
                    if (data.output) setLoginOutput(data.output)
                    if (data.deviceUrl) setDeviceUrl(data.deviceUrl)
                    ...
                    setLoginOutput(`Error: ${err?.message || 'Auth failed'}`)
                  } finally {
                    setLoginBusy(false)
                  }
                }}
              >
                {loginBusy ? 'Running auth...' : 'Start Auth (Device Code)'}
              </Button>
              {deviceUrl && (
                <div className="mt-2 space-y-1">
                  <p className="text-xs text-muted-foreground">Open this link to authenticate:</p>
            ...
            <p className="text-xs text-muted-foreground">
              Or set <code className="text-[11px] bg-black/20 px-1 rounded">OPENAI_API_KEY</code> as an environment variable to skip auth entirely.
            </p>
          </div>
